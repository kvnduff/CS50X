...
def index():
    ...
    for i in range(8):
        match.append(
            db.execute("SELECT m? FROM slave WHERE id = ?", i, sid)[0]["m" + str(i)]
        )
        nomatch.append(
            db.execute("SELECT n? FROM slave WHERE id = ?", i, sid)[0]["n" + str(i)]
        )
    ...
    solution = db.execute("SELECT solution FROM master WHERE id = ?", mid)[0][
        "solution"
    ]
    for i in range(5):
        master.append(
            db.execute("SELECT m? FROM master WHERE id = ?", i, mid)[0]["m" + str(i)]
        )
        cypher.append(
            db.execute("SELECT c? FROM master WHERE id = ?", i, mid)[0]["c" + str(i)]
        )
        solution_el.append(
            db.execute("SELECT s? FROM master WHERE id = ?", i, mid)[0]["s" + str(i)]
        )
...
def regex():
    ...
    for i in range(8):
        match.append(
            db.execute("SELECT m? FROM s_active WHERE first_row = 1", i)[0][
                "m" + str(i)
            ]
        )
        nomatch.append(
            db.execute("SELECT n? FROM s_active WHERE first_row = 1", i)[0][
                "n" + str(i)
            ]
        )

    # Use Python regex search function to test if regex matches match elements
    # (true) and doesn't match nomatch elements (false)
    match_true = []
    nomatch_false = []
    for i in range(8):
...
def reload():
    ...
    for i in range(8):
        match_new.append(
            db.execute("SELECT m? FROM slave WHERE id = ?", i, id)[0]["m" + str(i)]
        )
        nomatch_new.append(
            db.execute("SELECT n? FROM slave WHERE id = ?", i, id)[0]["n" + str(i)]
        )
